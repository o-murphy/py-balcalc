...
class ProfileCartridge(QtWidgets.QWidget, Ui_cartridge):
    ...
    def _temp_changed(self, value):
        self._cur_profile._temp = self._cur_units.tempUnits.currentData()(value)

    def _validate_mv(self, value):
        if self.mv.value() == 0:
            self.mv.setFocus()
        else:
            self._cur_profile.mv = self._cur_units.vUnits.currentData()(value)
    ...
    def setUnits(self):
        ...
        if self._cur_profile:
            ...
            vu = self._cur_units.vUnits.currentData()
